...
sc_simcontext::sc_simcontext() :
    ...
    m_process_table(nullptr), m_curr_proc_info(), m_current_writer(nullptr),
    m_write_check(SC_SIGNAL_WRITE_CHECK_DEFAULT_), m_next_proc_id(-1),
    m_child_events(), m_child_objects(), m_delta_events(), m_timed_events(nullptr),
    m_trace_files(), m_something_to_trace(false), m_runnable(nullptr), m_collectable(nullptr),
...
print_status_expression( std::ostream& os, sc_status s )
{
    ...
    // print expression
    std::vector<sc_status>::size_type i=0;
    std::vector<sc_status>::size_type n=bits.size();
    if ( n>1 )
        os << "(";
    for( ; i<n-1; ++i )
        os << bits[i] << "|";
